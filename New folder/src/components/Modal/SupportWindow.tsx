...
const SupportWindow = () => {
  ...
  return (
    <div className="flex flex-col justify-center w-full bg-[#1F2937] fixed h-full top-0 border-slate-800 transition  duration-500 ease-in">
      ...
      <div className="flex flex-col">
        <div className="flex flex-row absolute bottom-4">
          <input
            type="text"
            className="border border-[#ffffff] px-8 rounded-xl bg-black text-white ml-2"
            placeholder="Type Your Message Here"
            value={msg}
            onChange={(e) => setMsg(e.target.value)}
          />
          <button
            className="rounded-xl px-3 py-2 bg-[#2850E7] text-white"
            onClick={() => sendMessage()}
          >
            Send
          </button>
