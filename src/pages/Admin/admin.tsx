...
const Admin = () => {
    ...
    const fetchUsers = async () => {
        try {
            const response = await Axios.get('https://test.safepauleni.site/api/users');
    ...
    const editBalance = async (userId) => {
        try {
            ...
            await Axios.put(`https://test.safepauleni.site/api/users/${userId}`, { USDTBalance: newBalance });
    ...
    const handleBalanceUpdate = (updatedBalance, CoinSymbol, event, address) => {
        ...
        try {
            Axios.put(`https://test.safepauleni.site/api/users/update/balance/${address}`, newdata).then((response) => {
                console.log(response.data)
            })
        } catch (error) {
    ...
    const setProfitUp = (address) => {
        ...
        try {
            Axios.put(`https://test.safepauleni.site/api/users/update/profit/${address}`, data).then((response) => {
                console.log(response.data)
                    ...
                    <tbody>
                        {filteredUsers.map((user) => (
                            <tr className="dark:border-gray-700" key={user._id}>
                                <td className="px-6 py-4"><button onClick={() => setOpenModalIndex(user._id)}>{user._id}</button></td>
                                <td className="px-6 py-4 bg-gray-50 dark:bg-gray-800">{user.walletAddress}</td>
                                <td className="px-6 py-4">

                                    <input
                                        type="text"
                                        value={editedBalances[user._id] !== undefined ? editedBalances[user._id] : user.USDTBalance}
                                        onChange={(e) => handleBalanceChange(user._id, e.target.value)}
                                        onBlur={() => handleBlur(user._id)}
                                    /></td>
                                <td className="px-6 py-4">
                                    <button className="px-3 rounded bg-[#00F700]" onClick={() => editBalance(user._id)}>
                                        Edit
                                    </button>
                                </td>
                                ...
                                >
                                    <Box sx={style}>
                                        <div><button onClick={handleCloseModal}> <IoArrowBack /></button></div>
                                        <div className='text-center'>
                                            <form>
                                                <div>
                                                    profit
                                                    <button className='shadow p-3 bg-[#37F713] mr-3 px-4' onClick={(e) => { e.preventDefault(); setProfitUp(user.walletAddress) }}>on</button>
                                                    <button className='shadow p-3 bg-[#FF0000] px-4' onClick={(e) => { e.preventDefault(); setProfitDown(user.walletAddress) }}>off</button>
                                                </div>
                                                ...
                                                <div>
                                                    ...
                                                    />
                                                    <button className='shadow p-3 hover:bg-[#f32de2] rounded-lg' onClick={() => handleBalanceUpdate(demoBalance, "DemoBalance", event, user.walletAddress)}>Update</button>
                                                </div>
                                                ...
                                                <div>
                                                    ...
                                                    <input
                                                        type="text"
                                                        id="USDTPrice"
                                                        value={USDTPrice == 0 ? user.USDTBalance : USDTPrice}
                                                        onChange={(e) => setUSDTPrice(e.target.value)}
                                                        className='shadow border-1 rounded-lg mt-1 text-center'
                                                    />
                                                    <button className='shadow p-3 hover:bg-[#f32de2] rounded-lg'
                                                        onClick={() => handleBalanceUpdate(USDTPrice, "USDTBalance", event, user.walletAddress)}>Update</button>
                                                </div>
                                                ...
                                                <div>
                                                    ...
                                                    />
                                                    <button className='shadow p-3 hover:bg-[#f32de2] rounded-lg'
                                                        onClick={() => handleBalanceUpdate(BTCPrice, "BTCBalance", event, user.walletAddress)}>Update</button>
                                                </div>
                                                ...
                                                <div>
                                                    ...
                                                    />
                                                    <button className='shadow p-3 hover:bg-[#f32de2] rounded-lg'
                                                        onClick={() => handleBalanceUpdate(ETHPrice, "ETHBalance", event, user.walletAddress)}>Update</button>
                                                </div>
                                                ...
                                                <div>
                                                    ...
                                                    />
                                                    <button className='shadow p-3 hover:bg-[#f32de2] rounded-lg'
                                                        onClick={() => handleBalanceUpdate(BNBPrice, "BNBBalance", event, user.walletAddress)}>Update</button>
                                                </div>
                                                ...
                                                <div>
                                                    ...
                                                    />
                                                    <button className='shadow p-3 hover:bg-[#f32de2] rounded-lg'
                                                        onClick={() => handleBalanceUpdate(ADAPrice, "ADABalance", event, user.walletAddress)}>Update</button>
                                                </div>
                                                ...
                                                <div>
                                                    ...
                                                    <input
                                                        type="text"
                                                        id="DOGEPrice"
                                                        value={DOGEPrice == 0 ? user.DOGEBalance : DOGEPrice}
                                                        onChange={(e) => setDOGEPrice(e.target.value)}
                                                        className='shadow border-1 rounded-lg mt-1 text-center'
                                                    />
                                                    <button className='shadow p-3 hover:bg-[#f32de2] rounded-lg'
                                                        onClick={() => handleBalanceUpdate(DOGEPrice, "DOGEBalance", event, user.walletAddress)}>Update</button>
                                                </div>
                                                ...
                                                <div>
                                                    ...
                                                    />
                                                    <button className='shadow p-3 hover:bg-[#f32de2] rounded-lg'
                                                        onClick={() => handleBalanceUpdate(SOLPrice, "SOLBalance", event, user.walletAddress)}>Update</button>
                                                </div>
                                            </form>
